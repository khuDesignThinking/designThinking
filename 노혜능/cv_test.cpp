...
int detectEye(cv::Mat& im, cv::Mat& tpl, cv::Rect& rect)
{
	...
	face_cascade.detectMultiScale(im, faces, 1.1, 2, 0 | cv::CASCADE_SCALE_IMAGE, cv::Size(30, 30));
	
	for (int i = 0; i < faces.size(); i++)
	{
		cv::Mat face = im(faces[i]);
		eye_cascade.detectMultiScale(face, eyes, 1.1, 2, 0 | cv::CASCADE_SCALE_IMAGE, cv::Size(20, 20));
...
void trackEye(cv::Mat& im, cv::Mat& tpl, cv::Rect& rect)
{
	...
	cv::matchTemplate(im(window), tpl, dst, cv::TM_SQDIFF_NORMED);
